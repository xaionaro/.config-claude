...
- **Optimize for correctness over convenience.** Choose the correct, clean solution even when it's harder than the simple, practical, or convenient one. Take shortcuts only when explicitly authorized.
- **Use authoritative sources over generation**: When a file has a canonical source (LICENSE, .gitignore templates, config schemas, etc.), download or reference it instead of generating from memory.
