...
case "$VERDICT" in
  ...
  fail)
    ...
    if [ -n "$PERSISTENT_RULES" ]; then
      ...
      # Safety valve: surfaces the bypass hint at streak >= 10 in case the
      # persistent-rule heuristic missed a genuinely-stuck case (rules
      # paraphrased differently each time, filter normalization gap, etc.).
      # The streak itself isn't proof of unproductive work — it just caps
      # how long the agent can be held without seeing the escape path.
      OVERRIDE_HINT=$(printf '\n\nSafety hatch — %d consecutive blocks. The same-rule heuristic did not surface a bypass; this is a defensive fallback in case the heuristic has a hole. Bypass available: touch %s' "$STREAK" "$BYPASS_MARKER")
    else
      OVERRIDE_HINT=""
    fi
